...
-- ── 19. Multi-session refresh tokens ───────────────────────────
-- Replaces the old single-column users.refresh_token (overwritten on every
-- login, which silently signed out every other device). One row per active
-- session; a login no longer kills other sessions unless explicitly revoked
-- (role switch, password reset, admin disable).
